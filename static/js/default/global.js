...
function renderTemplate(template, list) {
	...
	var buffer = [];
	var temp = '';
	for (var i = 0; i < list.length; i++) { //替换 list 中的内容，并输出替换后的内容.
		temp = template;
...
function loadMoreMessages(forceToShow) { //forceToShow = true
	var MessagesList = $("#MessagesList");//真正显示的 view
	var InboxID = $("#InboxID").val(); //私信的 id
	var MessagesPage = $("#MessagesPage");// page 1
	var MessagesLoading = $("#MessagesLoading"); // 0
	...
	if (forceToShow || MessagesLoading.val() !== "1") {
		...
		$.ajax({
			...
			success: function(Result) {
				...
				if (Result.Status === 1) {
					var Template = $("#MessageTemplate").html();	//JavaScript 中需要显示一条的 view 的 Template .
					for (var i = Result.MessagesArray.length - 1; i >= 0; i--) {//确定个人头像的位置
